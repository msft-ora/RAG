...
def add_merchant_sliding_counts(
    ...
    where w = Window.partitionBy(<spec merchant_col>)
    ...
    Each spec: {"out_col": ..., "merchant_col": ..., "col_exp": ...,
      ...
      - merchant_col: THIS output's partition key - a column name or a list,
    ...
    for i, s in enumerate(specs):
        for key in ("out_col", "merchant_col", "col_exp", "window_seconds"):
            if key not in s:
        ...
        seen.add(s["out_col"])
        key_tuple = _norm_keys(s["merchant_col"], df.columns)
